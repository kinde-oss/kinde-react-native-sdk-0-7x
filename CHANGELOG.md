...
- **Minimum iOS version is now 12.0** — This SDK now uses `react-native-app-auth` for OAuth authentication, which depends on [AppAuth-iOS 2.0.0](https://github.com/openid/AppAuth-iOS/releases/tag/2.0.0). If your app previously targeted iOS versions below 12.0, update your `IPHONEOS_DEPLOYMENT_TARGET` in Xcode.
...
- **`AuthBrowserOptions`**: Most options from the previous `react-native-inappbrowser-reborn` integration are now deprecated. See migration guide below.

##### Migration Guide

The SDK now uses `react-native-app-auth` instead of `react-native-inappbrowser-reborn`. Your existing code will continue to work, but some `AuthBrowserOptions` are deprecated:

| Old Option | Status | Replacement |
|------------|--------|-------------|
| `ephemeralWebSession` | Deprecated | Use `iosPrefersEphemeralSession` |
| `browserPackage` | Deprecated | Use `androidAllowCustomBrowsers` |
| `animated`, `animations`, `dismissButtonStyle`, `enableBarCollapsing`, `enableDefaultShare`, `enableUrlBarHiding`, `forceCloseOnRedirection`, `formSheetPreferredContentSize`, `hasBackButton`, `headers`, `includeReferrer`, `modalEnabled`, `modalPresentationStyle`, `modalTransitionStyle`, `navigationBarColor`, `navigationBarDividerColor`, `preferredBarTintColor`, `preferredControlTintColor`, `readerMode`, `secondaryToolbarColor`, `showInRecents`, `showTitle`, `toolbarColor`, `waitForRedirectDelay` | Deprecated | No equivalent (ignored) |

**Active options** (used by `react-native-app-auth`):
- `iosPrefersEphemeralSession` — Use private browser session on iOS
- `iosCustomBrowser` — Specify iOS browser (`'safari'`, `'chrome'`, `'opera'`, `'firefox'`)
- `androidAllowCustomBrowsers` — Allowed Android browsers
